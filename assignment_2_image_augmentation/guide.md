...
- saves each execution in a new numbered folder inside `augmented_outputs`
...
The script will generate several modified versions of the default `Profile.jpg` image and save them automatically.

To use a different image, provide its path after the script name:

```bash
python image_augmentation_assignment.py "path/to/another_image.jpg"
```

For example, from this assignment folder, an image in the repository root can be used with:

```bash
python image_augmentation_assignment.py "..\another_image.jpg"
```

Every run creates a separate output folder, so previous results are not overwritten:
...
Use `run_001` for the first image, `run_002` for the second image, and so on.
